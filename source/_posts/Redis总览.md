...
应用程序和数据库分别部署在两台机器上，通常使用socket网络通信，由于需要网络通信，性能可能还会比单机架构低一些，分离主要解决资源隔离、独立扩展和故障隔离，跨机器通信反而会增加网络开销。
...
其中涉及的技术包括：使⽤ memcached 独立部署、通过网络访问的分布式缓存，使⽤ Redis 作为分布式缓存，还会涉及缓存一致性、缓存穿透/击穿、缓存雪崩、热点数据集中失效等问题
